...
create table if not exists CASES(SNO int,NAME_OF_CITY varchar(17),ACTIVE_CASES INT(37),RECOVERED_CASES INT(40),DEATHS INT(35));
...
create table if not exists user_data(password varchar(30) default'000');
''')
conn.commit()


"""
c.execute('insert into user_data values("root");')
conn.commit()
...
def hospitals(name,table,SNO):
    ...
    A=pd.read_sql('SELECT NAME_OF_HOSPITAL FROM {}'.format(table),conn)
    Width=os.get_terminal_size()
    pd.set_option('display.width',Width[0])
    print(A)
...
def updateCity(table,SNO):
    e=input('''What to update:
1.Hospital info
2.Different bed prices
3.No. of cases
...
def updateBedPrices(table,SNO):
    e=input('''UPDATE BED PRICES:
1.ISOLATION BEDS
2.ICU BEDS WITHOUT VENTILATOR
3.ICU BEDS WITH VENTILATOR
...
def changePass():
    ...
    if cp==c.execute('SELECT password from user_data;'):
        np=input('ENTER NEW PASSWORD:')
        c.execute('UPDATE user_data set password={};'.format(np))
        conn.commit()
